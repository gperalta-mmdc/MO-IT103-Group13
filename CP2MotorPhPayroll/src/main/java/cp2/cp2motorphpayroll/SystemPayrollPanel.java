...
public class SystemPayrollPanel {
    ...
    static void show() {
        ...
        JPanel header = SystemGUIHelper.buildHeader(
            "Payroll Staff Portal", frame, 1000,
        ...
        yearSpinner.setToolTipText("Select year 2020 – " + currentYear);
        ...
        monthCombo.setBackground(SystemGUIHelper.COLOR_FIELD_BG);
        ...
        JRadioButton oneEmpBtn = new JRadioButton("One Employee");
        JRadioButton allEmpBtn = new JRadioButton("All Employees");
        ...
        c.gridx = 3;
        JTextField empNameField = SystemGUIHelper.makeField(14);
        ...
        JButton summaryBtn = SystemGUIHelper.makeButton(
            "Compute Salaries", new Color(30, 80, 160));
        summaryBtn.setPreferredSize(new Dimension(200, 32));
        controls.add(summaryBtn, c);

        c.gridx = 4; c.gridwidth = 3;
        JButton clearTableBtn = SystemGUIHelper.makeButton(
            "Clear Table", new Color(120, 120, 120));
        clearTableBtn.setPreferredSize(new Dimension(130, 32));
        controls.add(clearTableBtn, c);
        
         c.gridx = 2; c.gridwidth = 3;
        JButton gensumBtn = SystemGUIHelper.makeButton(
            "Generate Payroll Summary", new Color(30, 80, 160));
        ...
        table.getTableHeader().setForeground(SystemGUIHelper.COLOR_SECONDARY);
        ...
        statusBar = new JLabel(
            "  Ready. Select options and click Generate Summary.");
        ...
        allEmpBtn.addActionListener(e -> {
           ...
           // EVENT - Generate Payroll Summary button
        ...
        empNumField.addFocusListener(new FocusAdapter() {
            public void focusLost(FocusEvent e) {
                ...
                if (!num.isEmpty() && num.matches("\\d+")) {
                    String[] data = EntryPoint.employeeMap.get(num);
        ...
        clearTableBtn.addActionListener(e -> {
            ...
            statusBar.setText("  Table cleared.");
        });
        ...
        summaryBtn.addActionListener(e -> {
            ...
            for (int i = 0; i < monthNames.length; i++) {
                if (monthNames[i].equals(selectedMonth)) {
                    monthIndex = i; break;
                }
            }
            if (monthIndex == -1) {
            ...
            java.util.List<Integer> monthsToProcess =
    ...
    private static void genPayrollSummary(int selectedYear) {
        ...
        for (String empNum : keys) {
            ...
            if (data == null) continue; // skip invalid, don't halt loop

            double hourlyRate = parseDouble(
                DataProcessing.safeGet(data, 16));
            ...
            for (int m = 1; m <= 12; m++) {
                ...
                double gross1st = computeGrossPay(
                    new double[]{ hours1st, hourlyRate });
                double gross2nd = computeGrossPay(
                    new double[]{ hours2nd, hourlyRate });

                double totalMonthlyGross = gross1st + gross2nd;
                totalGross += totalMonthlyGross;

                double[] deductions = computeDeductions(
                    new double[]{ totalMonthlyGross });
                double totalDed = deductions[4];
                totalDeductions += totalDed;

                double netPay2nd = computeNetPay(
                    new double[]{ gross2nd, totalDed });
        ...
        String display =
            ...
            + "Average Net Pay:                 ₱"
                + String.format("%,.2f", totalNet / empAmt) + "\n" + "\n"; 

        JOptionPane.showMessageDialog(
            frame,
            display,
            "MotorPH Payroll Summary — " + selectedYear,
            JOptionPane.INFORMATION_MESSAGE);
    }
    
    
    //  ACTION BAR - delegates CRUD to SystemDataOperations
    private static JPanel buildActionBar() {
        ...
        viewBtn.setPreferredSize(new Dimension(140, 28));

        JButton addBtn = SystemGUIHelper.makeButton("Add Employee",
                SystemGUIHelper.COLOR_SUCCESS);
        addBtn.setPreferredSize(new Dimension(130, 28));

        JButton editBtn = SystemGUIHelper.makeButton("Edit Employee",
                new Color(255, 165, 100));
        editBtn.setPreferredSize(new Dimension(130, 28));

        JButton deleteBtn = SystemGUIHelper.makeButton("Delete Employee",
                new Color(180, 30, 30));
        deleteBtn.setPreferredSize(new Dimension(150, 28));

        // All CRUD buttons always enabled
        viewBtn.addActionListener(e ->
            SystemDataOperations.showEmployeePicker("View Employee",
                empNum -> SystemDataOperations.showViewDialog(empNum)));

        addBtn.addActionListener(e ->
            SystemDataOperations.showAddDialog());

        editBtn.addActionListener(e ->
            SystemDataOperations.showEmployeePicker("Edit Employee",
                empNum -> SystemDataOperations.showEditDialog(empNum)));

        deleteBtn.addActionListener(e ->
            SystemDataOperations.showEmployeePicker("Delete Employee",
                empNum -> SystemDataOperations.confirmAndDelete(empNum)));
    ...
    static void addPayrollRows(String empNum,
                                String yearMonth, int selectedYear) {
        String[] data = EntryPoint.employeeMap.get(empNum);
        if (data == null) return;
        ...
        String monthDisplay = java.time.Month.of(ymMonth)
            .getDisplayName(java.time.format.TextStyle.FULL,
                java.util.Locale.ENGLISH) + " " + ymYear;
        String cutoff2nd = "2nd (16–" + lastDay + ")";

        // 1st cutoff row - gross only, no deductions
        ...
        // 2nd cutoff row - deductions applied
        tableModel.addRow(new Object[]{
            empNum, lastName, firstName, monthDisplay,
            cutoff2nd,
    ...
    static double parseDouble(String s) {
        try { return Double.parseDouble(s.replace(",", "")); }
            ...
            System.out.println("[Warning] Could not parse: \"" + s + "\"");
            return 0;
        }
    }

    // Gross pay
    static double computeGrossPay(double[] params) {
        return params[0] * params[1];
    }

    // SSS contribution
    static double computeSSS(double[] params) {
        double g = params[0];
        double[][] sssTable = {
            ...
            { Double.MAX_VALUE, 1125.00 }
        };
        for (double[] bracket : sssTable)
            if (g <= bracket[0]) return bracket[1];
        return 1125.00;
    }

    // PhilHealth
    static double computePhilHealth(double[] params) {
        return Math.round(
            (params[0] * PHILHEALTH_RATE / 2) * 100.0) / 100.0;
    }

    // Pag-IBIG
    static double computePagIBIG(double[] params) {
        return params[0] <= 1_500 ? 100.0 : 200.0;
    }

    // Withholding tax
    static double computeWithholdingTax(double[] params) {
        double s = params[0];
        double tax;
        if      (s <= 20_833)  tax = 0;
        else if (s <= 33_333)  tax = (s - 20_833)  * 0.20;
        else if (s <= 66_667)  tax = 2_500   + (s - 33_333)  * 0.25;
        else if (s <= 166_667) tax = 10_833  + (s - 66_667)  * 0.30;
        else if (s <= 666_667) tax = 40_833  + (s - 166_667) * 0.32;
        else                    tax = 200_833 + (s - 666_667) * 0.35;
        return Math.round(tax * 100.0) / 100.0;
    }

    // Total deductions
    static double[] computeDeductions(double[] params) {
        double g       = params[0];
        double sss     = computeSSS(new double[]{ g });
        double ph      = computePhilHealth(new double[]{ g });
        double pagIbig = computePagIBIG(new double[]{ g });
        double tax     = computeWithholdingTax(new double[]{ g });
        return new double[]{ sss, ph, pagIbig, tax,
            sss + ph + pagIbig + tax };
    }

    // Net pay
    static double computeNetPay(double[] params) {
        return params[0] - params[1];
    }
    ...
    private static double computeHoursWorked(String empID,
        ...
        java.util.List<String[]> records =
            EntryPoint.attendanceMap.get(empID);
    ...
    private static double computeHours(String logIn, String logOut) {
        try {
            ...
            java.time.LocalTime start  =
                java.time.LocalTime.parse(logIn.trim(),  fmt);
            java.time.LocalTime end    =
                java.time.LocalTime.parse(logOut.trim(), fmt);
            java.time.LocalTime oStart = java.time.LocalTime.of(8, 0);
            java.time.LocalTime grace  = java.time.LocalTime.of(8, 5);
            java.time.LocalTime oEnd   = java.time.LocalTime.of(17, 0);

            if (start.isBefore(oStart))  start = oStart;
            if (!start.isAfter(grace))   start = oStart;
            if (end.isAfter(oEnd))       end   = oEnd;
    ...
    // Extracts "YYYY-MM" from "M/D/YYYY"
    ...
    // Extracts day-of-month integer from "M/D/YYYY"
